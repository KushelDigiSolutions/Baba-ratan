...
export async function generateMetadata({ params }) {
    ...
    try {
        const response = await fetch(`https://bajrangastro.kdscrm.com/api/gemstones/${categorySlug}/${productSlug}`);
        const data = await response.json();
        const product = data.product || data.data || data;

        return {
            title: `${product.name || 'Product Details'} | Bajrang Astro`,
            description: product.meta_description || product.description?.replace(/<[^>]*>/g, '').slice(0, 160) || 'View detailed information about our gemstones.',
        };
    } catch (error) {
        return {
        ...
        return (
            <main className="min-h-screen">
                ...
                <div className="max-w-7xl mx-auto px-20 py-20 text-center">
                    ...
                    <p className="mt-4 text-gray-600">The product you are looking for might have been moved or deleted.</p>
                </div>
            </main>
        );
    }
